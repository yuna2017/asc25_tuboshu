...
% Document info
\title{
    \includegraphics[width=0.4\textwidth]{images/logo/ysu-logo.png}\\[2cm] % logo
...
Analysis of SRR23538290 (93.55M reads) and SRR23538291 (136.51M reads) revealed consistent processing patterns. Cutadapt preprocessing achieved 23.91-23.83M reads/min throughput, removing 4.74M (5.07%) and 6.84M (5.02%) reads respectively. HISAT-3N alignment demonstrated:
\begin{itemize}
\item \textbf{ncRNA filtering}: 14.40% (SRR23538290) and 14.77% (SRR23538291) alignment rates
\item \textbf{Genome alignment}: 91.52% (69.60M) and 90.11% (99.59M) mapping efficiencies
\end{itemize}

Critical path analysis revealed resource utilization patterns:
\begin{itemize}
\item \textbf{CPU-bound operations}: HISAT-3N alignment maintained 94.3% thread utilization (56 threads)
...
\begin{itemize}
\item 21 runtime metrics including thread utilization (88.7%±9.2%)
\item Memory pressure profiles (peak RSS 59.1GB)
\item I/O wait states (max 14.2% during BAM sorting)
\end{itemize}
